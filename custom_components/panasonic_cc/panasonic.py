...
class PanasonicApiDevice:
    ...
    async def do_update(self):
        ...
        try:
            ...
            _LOGGER.debug(
                "Error trying to get device {id} state, probably expired token, trying to update it...".format(**self.device))
            try:
                await self.hass.async_add_executor_job(self._api.login)
                data = await self.hass.async_add_executor_job(self._api.get_device, self.id)
            except:
                _LOGGER.debug(
                    "Failed to renew token for device {id}, giving up for now".format(**self.device))
                return

        if data is None:
            self._available = False
            _LOGGER.debug(
                "Received no data for device {id}".format(**self.device))
            return
        try:
            ...
            if 'ecoFunctionData' in plst:
                self._ecofunctiondata = plst['ecoFunctionData']

        except Exception as e:
            _LOGGER.debug(
                "Failed to set data for device {id}".format(**self.device))
    ...
    async def do_update_energy(self):
        ...
        if data is None:
            _LOGGER.debug(
                "Received no energy data for device {id}".format(**self.device))
            return
        t1 = datetime.now()
    ...
    @property
    def ecofunctiondata(self):
        return self._ecofunctiondata
    ...
    async def set_ecofunctiondata(self, ecofunctiondata):
        """Set new eco function data."""
        _LOGGER.debug("Set %s eco function data %s",
                      self.name, ecofunctiondata)

        await self.hass.async_add_executor_job(
            self.set_device,
            {"ecoFunctionData":
                self.constants.EcoFunctionData[ecofunctiondata]}
        )
        await self.do_update()
