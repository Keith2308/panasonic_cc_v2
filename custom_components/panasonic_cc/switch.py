...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for device in hass.data[PANASONIC_DEVICES]:
        ...
        devices.append(PanasonicEcoFunctionDataSwitch(device))
...
class PanasonicEcoFunctionDataSwitch(ToggleEntity):
    """Representation of a zone."""
    ...
    def unique_id(self):
        """Return a unique ID."""
        return f"{self._api.id}-ecofunctiondata"
    ...
    def name(self):
        """Return the name of the sensor."""
        return f"{self._api.name} Eco Function Data"

    @property
    def is_on(self):
        """Return the state of the sensor."""
        return self._api.ecofunctiondata == self._api.constants.EcoFunctionData.On
    ...
    async def async_turn_on(self, **kwargs):
        """Turn on eco function."""
        await self._api.set_ecofunctiondata(self._api.constants.EcoFunctionData.On.name)

    async def async_turn_off(self, **kwargs):
        """Turn off eco function."""
        await self._api.set_ecofunctiondata(self._api.constants.EcoFunctionData.Off.name)
